...
namespace ThrilJunkyServices.Repositories
{
    public class PostRepository : IPostRepository
    {
        ...
        public List<Post> GetAllByLocationOrderByCreatedDate(float latitude, float longitude, int radius)
        {
            using(IDatabase db = Connection)
            {
                return db.Fetch<Post>($"SELECT TOP 50 p.*, l.* FROM Post p INNER JOIN Location l on l.LocationId = p.LocationId WHERE dbo.GetDistanceBetween({latitude}, {longitude}, l.latitude, l.longitude) <= {radius};");
            }
        }
